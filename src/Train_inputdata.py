...
def get_files(file_dir):
    ...
    for file in os.listdir(file_dir):   # file就是要读取的照片
        name = file.split(sep='_')      # 因为照片的格式是gesture_1_1.jpg/gesture_1_2.jpg
        if name[1] == '1':            # 所以只用读取两个_中间这个字符串
    ...
    temp = np.array([image_list, label_list])  # 生成一个两行数组列表，大小是2 X (2000X5)
    temp = temp.transpose()   # 转置向量，大小变成(2000X5) X 2
